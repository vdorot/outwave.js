...
define(['./vendor/jdataview'],function(_jDataView){
    ...
    DataFile.prototype = {
        ...
        getSum1Sample: function(sample,channel){
            ...
            var target = (sample + 1) * (this.sum1p + 1) - 1 + Math.floor(((sample+1) * this.sum1p - 1) / this.sum2p);
        ...
        getSum2Sample: function(sample,channel){
            var target  = (sample + 1) * (this.sum2p + 1) - 1 + Math.floor((sample + 1) * this.sum2p / this.sum1p);
            return this.sampleSource(target,channel);
        },
        ...
        getNoSumSample: function(sample,channel){
            return this.sampleSource(sample + Math.floor(sample / this.sum1p) + Math.floor(sample / this.sum2p), channel);
        },
